<x-filament-breezy::grid-section md=2 :title="__('filament-breezy::default.profile.2fa.title')"
    ...
    <x-filament::card>
        ...
        @if($this->showRequiresTwoFactorAlert())
            ...
            <div
                style="{{ \Illuminate\Support\Arr::toCssStyles([\Filament\Support\get_color_css_variables('danger',shades: [300, 400, 500, 600])]) }}"
                    ...
                    <div class="px-4 space-y-3">
                        ...
                        <div>
                            @foreach ($this->recoveryCodes->toArray() as $code )
                                <span
                                    class="inline-flex items-center p-1 text-xs font-medium text-slate-800 dark:text-slate-400 bg-slate-100 rounded-full dark:bg-slate-900">{{ $code }}</span>
                            @endforeach
                        </div>
